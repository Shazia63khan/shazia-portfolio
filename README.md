 Hi, I'm Shazia Bano
🎓 B.Tech Computer Science Engineering Student
🤖 Aspiring AI & Machine Learning Engineer
🌐 Web Development Enthusiast

🚀 About Me

I am a Computer Science Engineering student passionate about Artificial Intelligence, Machine Learning, and Web Development. I enjoy building real-world projects, working with data, and contributing to innovative tech solutions. Currently, I am gaining hands-on experience with Large Language Models (LLMs) through my internship.

💼 Experience

🧠 LLM Post-Training Intern — Ethara.ai
Jan 2026 – Present
Working on post-training pipelines for Large Language Models
Fine-tuning and optimizing LLM performance

🌐 Web Development Intern — Vault Of Codes
Feb 2025 – Mar 2025
Completed web development internship
Built a Quiz Application
Solved multiple real-world development tasks

🎓 Education
B.Tech in Computer Science Engineering
Vivekananda College of Technology and Management
Dr. APJ Abdul Kalam Technical University, Lucknow
📊 CGPA: 8.44 (2024 – Present)

🛠️ Tech Stack

Languages & Tools:
Python (NumPy, Pandas, Scikit-Learn)
SQL, C, JavaScript
HTML, CSS
Firebase

Core Skills:
Machine Learning Basics
Data Analysis
Web Development
Team Collaboration


🧪 Projects

🔗 HelpHub (Ongoing)
A disaster relief web platform connecting victims, NGOs, and donors.
Real-time help requests
Resource & volunteer management
Firebase authentication & database
Focus on transparency & efficiency

🍳 Recipe Hub
A responsive recipe management web app.
Built with HTML, CSS, JavaScript
Smooth user experience
Mobile-friendly design

📜 Certifications
Design and Analysis of Algorithms — NPTEL (IIT Madras)
AI Agent Creation — GeeksforGeeks & Salesforce
Foundations of Data Science — Analytics Vidhya
AI Tools for Programmers — Physics Wallah
Power BI in Action — Wayspire

📫 Let's Connect
📧 Email: shaziaofficial.00@gmail.com
💼 LinkedIn: linkedin.com/in/shazia-bano-896030322
⭐ How to Use
This repository contains highlights of my skills, experience, and projects. Feel free to connect with me for collaborations, internships, or opportunities!
